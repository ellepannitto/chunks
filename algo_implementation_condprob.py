...
fout = open('adventure_output_condprob.tsv', 'w')

#cerco coppia a, b con frequenza massima
hash_table = {x:y for x, y in hash_table.items() if y.freq > 0}
sorted_hash = sorted(hash_table.items(), key = lambda x: -x[1].freq*x[1].freq/charfreq[x[0][0]])
most_frequent_record = sorted_hash[0][1]
#print(sorted_hash[:2])
#print(most_frequent_record.freq, charfreq[most_frequent_record.pair[0]])
#input()
